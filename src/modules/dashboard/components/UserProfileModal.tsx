...
export function UserProfileModal({ trigger, open: controlledOpen, onOpenChange }: Props) {
  ...
  const confirmWord = t('profile.deleteAccountConfirmPlaceholder')
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      toast({ title: t('profile.deleteAccountError'), variant: 'destructive' })
    } finally {
      setIsDeleting(false)
    }
  }
      ...
      >
        <DialogContent className="sm:max-w-sm">
          ...
          <div className="space-y-1.5 py-1">
            <Label htmlFor="delete-confirm">{t('profile.deleteAccountConfirmLabel')}</Label>
            <Input
              id="delete-confirm"
              value={deleteConfirm}
          ...
          <DialogFooter>
            ...
            <Button
              type="button"
              variant="destructive"
              disabled={deleteConfirm !== confirmWord || isDeleting}
