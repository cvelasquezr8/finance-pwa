...
export function UserProfileModal({ trigger, open: controlledOpen, onOpenChange }: Props) {
  ...
  const { register, handleSubmit, setValue, watch, reset, formState: { errors, isSubmitting } } = useForm<ProfileFormValues>({
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      {!isControlled && (
        <DialogTrigger asChild>
          {trigger ?? (
            <button className="flex items-center gap-2 w-full px-3 py-1 rounded-lg hover:bg-accent transition-colors">
              <div className="flex h-7 w-7 items-center justify-center rounded-full bg-primary/15 dark:bg-primary/30 text-xs font-semibold text-primary flex-shrink-0 overflow-hidden">
                {currentAvatar
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="flex justify-center py-2">
            <div className="relative group">
              <div className="h-24 w-24 rounded-full overflow-hidden bg-primary/15 dark:bg-primary/30 flex items-center justify-center ring-2 ring-border">
                {avatarPreview
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              {errors.firstName && <p className="text-xs text-destructive">{t('profile.minChars')}</p>}
            </div>
            <div className="space-y-1.5">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              {errors.lastName && <p className="text-xs text-destructive">{t('profile.minChars')}</p>}
            </div>
            <div className="space-y-1.5">
          ...
          <div className="space-y-1.5">
            ...
            <Input id="p-phone" placeholder={t('profile.phonePlaceholder')} {...register('phone')} />
          </div>

          <div className="space-y-1.5">
            <Label htmlFor="p-email">{t('profile.email')}</Label>
            <Input id="p-email" type="email" {...register('email')} />
            {errors.email && <p className="text-xs text-destructive">{t('profile.invalidEmail')}</p>}
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1.5">
              ...
              <Select value={watch('timezone')} onValueChange={(v) => setValue('timezone', v)}>
                ...
                <SelectContent>
                  {TIMEZONES.map((tz) => (
                    <SelectItem key={tz} value={tz}>{tz.replace('America/', '').replace('Europe/', '')}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              <Select value={watch('language')} onValueChange={(v) => setValue('language', v as 'es' | 'en')}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
            ...
            <div className="space-y-1.5">
              ...
              <Select value={watch('theme')} onValueChange={(v) => setValue('theme', v as 'dark' | 'light')}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent>
          ...
          <DialogFooter>
            <Button type="button" variant="outline" onClick={() => setOpen(false)}>{t('profile.cancel')}</Button>
